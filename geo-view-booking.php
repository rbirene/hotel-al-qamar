...
$sql = "SELECT * FROM geotest3_tblBookingRequestDetails";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) { //while there are rows to process...
        echo "<p>Booking ID: " . $row["DBbID"]. "<br>Number of Nights Staying: " . $row["DBbNightsNo"]. "<br>Number of Adults: " . $row["DBbAdultNo"] . "<br>Requests: " . $row["DBbRequest"] . "</p>"; //show the values in that row
    }
} else { //otherwise...
